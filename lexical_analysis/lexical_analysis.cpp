...
void analysis2(char* inChar,int *c,int *s,int count_c[],int *j1,int *count_case1)
{
...
void analysis3(char* inChar,int *c,int *s,int count_c[],int *j1,int *count_case1,int *l1,int *count_if_else1,int *count_if_elseif_else1,string if_else[])
{
    
    int n=0,m,o=0;
    //string if_else[10]; //用来判断if-else和if-elseif-else结构的堆栈
    //用于遍历inChar字符数组
    int i = 0;
    while(inChar[i] != '\0')
    {
...
int main()
{
    ...
    if(myfile.is_open()){
        ...
        while(getline(myfile,line)){
            ...
            if(type==2){
               analysis2(inChar,&count,&count_s,count_c,&j,&count_case);
            }
            if(type==3|| type==4){
              analysis3(inChar,&count,&count_s,count_c,&j,&count_case,&l,&count_if_else,&count_if_elseif_else,if_else1);
                       }
        }
         myfile.close();
    }
